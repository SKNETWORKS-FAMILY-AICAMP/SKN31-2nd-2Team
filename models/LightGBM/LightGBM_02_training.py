...
# -*- coding: utf-8 -*-
"""
[2] LightGBM 모델 학습, 평가 스크립트
-------------------------------------------------------------------------
1. LightGBM 모델 생성 및 학습 (early_stopping 적용 포함)
2. 테스트 데이터를 활용한 모델 예측 및 평가 (정확도, ROC-AUC, 분류 리포트)
3. 피쳐 중요도(Feature Importance) 그래프 저장
"""
...
def run_model_training_and_evaluation(X_train, X_test, y_train, y_test, best_params=None):
    ...
    print("-----------------------------------------------------------------")
    print("모델 생성 및 학습 완료")
    print(f"최적의 트리 반복 횟수 (Best Iteration): {model.best_iteration_}")
    ...
    # 데이터 타입이 다른타입으로 인식될 상황을 대비하기 위해서 예측값과 정답지를 무조건 숫자 0과 1로 강제 통일.
    ...
    # 데이터타입이 완전한 숫자(0 또는 1) 상태이므로 에러없이 정상 연산.
    metrics = {
